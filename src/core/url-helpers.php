...
{
    /**
     * Parses the requested page and parameters from a path-like URI.
     *
     * - `action`: the first segment of the path
     * - `params`: the remainder of the path split on `/`
     */
    public static function getRequestedPage(string $path, string $defaultAction): array
    {
     ...
     * the key `my-key`.
     */
    public static function getValueFromPageParams(array $pageParams, string $key): string | null
    {
     ...
     */
    public static function parsePageNumber(array $pageParams): int
    {
        $page = self::getValueFromPageParams($pageParams, 'page');
        if (is_numeric($page)) {
            return (int)$page;
        }
...
{
    return UrlHelpers::getRequestedPage($path, $defaultAction);
}
...
{
    return UrlHelpers::getValueFromPageParams($pageParams, $key);
}
...
{
    return UrlHelpers::parsePageNumber($pageParams);
}
